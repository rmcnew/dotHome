...
#     REVISION:  $Id: //share/user/rmcnew/home/.vim/perl-support/scripts/pmdesc3.pl#1 $
    ...
    for my $inc_dir (sort { length $b <=> length $a } @ARGV) {
        find({
                wanted => sub {
                    ...
                    my $module  = get_module_name($File: //share/user/rmcnew/home/.vim/perl-support/scripts/pmdesc3.pl $inc_dir);
                    my $version;
                    if ( /\.pod\z/ )
                    {
                        $pm =~ s/\.pod\z/\.pm/; 
